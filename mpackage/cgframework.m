    ...
    Module[{irr, i, j, k, tot, res, res2, fun, tmp1, tmp2}, 
     irr = gi[KeyIrr]; For[i = 1, i <= Length[irr], i++, 
       For[j = i, j <= Length[irr], j++, res = Kronecker[gi, irr[[i,1]], 
        ...
        False, Continue[]]; fun = gi[KeyDotFunction]; 
      For[i = 1, i <= Length[irr], i++, 
       tmp1 = Table[RandomInteger[], {k, 1, GetDimensionByRep[gi, 
         ...
         tmp2 = Table[RandomInteger[], {k, 1, GetDimensionByRep[gi, 
              irr[[j]]]}]; res = Kronecker[gi, irr[[i,1]], irr[[j,1]]]; 
          For[k = 1, k <= Length[res], k++, res = fun[tmp1, tmp2, irr[[i,1]], 
              irr[[j,1]], res[[k]]]; Assert[Length[res] == GetDimensionByRep[
               gi, res[[k]]]]; ]; ]; ]; ]
     ...
     irr = gi[KeyIrr]; For[i = 1, i <= Length[irr], i++, 
      ...
      For[i = 1, i <= Length[irr], i++, 
       tmp1 = Table[RandomInteger[], {k, 1, GetDimensionByRep[gi, 
         ...
         tmp2 = Table[RandomInteger[], {k, 1, GetDimensionByRep[gi, 
              irr[[j,1]]]}]; res = Kronecker[gi, irr[[i,1]], irr[[j,1]]]; 
          Print["res=", res]; For[k = 1, k <= Length[res], k++, 
           res = fun[tmp1, tmp2, irr[[i,1]], irr[[j,1]], res[[k]]]; 
            Assert[Length[res] == GetDimensionByRep[gi, res[[k]]]]; ]; ]; ]; ]
