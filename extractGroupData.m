...
LoadGroupFromFile[filename_, opts:OptionsPattern[]] := 
    Module[{f, ret, name, value, keys = {}}, 
     f = StringToStream[ReadGAP[filename]]; While[True, 
       name = Read[f, Expression]; If[name == EndOfFile, Break[]]; 
        AppendTo[keys, name]; ret[name] = Read[f, Expression]; ]; 
      ret["Keys"] = keys; Return[ret]; ]
